...
function buildStructureFromTypeNode(
  typeNode: ts.TypeNode,
  sourceFile: ts.SourceFile,
  typeDeclarations: Map<string, TypeDeclaration>,
  visitingNames: Set<string>,
  ...
  if (ts.isTypeReferenceNode(coreTypeNode)) {
    ...
    if (typeName === "Array" && coreTypeNode.typeArguments?.[0]) {
      const item = buildStructureFromTypeNode(coreTypeNode.typeArguments[0], sourceFile, typeDeclarations, visitingNames, depth + 1);
      if (!item) {
        return undefined;
      }
      return { kind: "array", typeLabel, item, allowNull, allowUndefined };
    ...
    if (declaration && !visitingNames.has(typeName)) {
      ...
      const resolved = ts.isInterfaceDeclaration(declaration)
        ? buildStructureFromMembers(declaration.members, sourceFile, typeDeclarations, visitingNames, depth + 1, typeLabel, allowNull, allowUndefined)
  ...
  return buildPrimitiveStructure(typeLabel, buildControlFromTypeNode(typeNode, sourceFile, typeDeclarations), buildOptionsFromTypeNode(typeNode, sourceFile, typeDeclarations), allowNull, allowUndefined);
}
...
function buildStructureFromType(
  type: ts.Type,
  ...
  if (checker.isTupleType(coreType)) {
    const items = checker.getTypeArguments(coreType as ts.TypeReference)
